...
assert.equal(new Function(app.match(/function reagentDoorDropAction\(source, target, occupied\) \{[\s\S]*?\n    \}/)[0] + '; return reagentDoorDropAction;')()({ unitId: 'A', shelf: 1, slot: 1 }, { unitId: 'B', shelf: 2, slot: 3 }, true), 'swap');
assert.equal(new Function(app.match(/function reagentDoorDropAction\(source, target, occupied\) \{[\s\S]*?\n    \}/)[0] + '; return reagentDoorDropAction;')()(null, { unitId: 'A', shelf: 1, slot: 1 }, true), 'replace');
assert.match(app, /data-door-slot/);
assert.match(app, /doorShelves: Math\.round/);
