...
assert.match(assistant, /rhineLabAssistantDock/);
assert.match(assistant, /function characterDockEdge[\s\S]*EDGE_SNAP/);
assert.match(assistant, /function dockCharacter[\s\S]*is-edge-docked/);
assert.match(assistant, /function undockCharacter[\s\S]*removeItem\(DOCK_KEY\)/);
assert.match(assistant, /docked: toggle\.classList\.contains\('is-edge-docked'\)[\s\S]*dockEdge: toggle\.dataset\.edge/);
assert.match(assistant, /dragState\.docked \? \(dragState\.dockEdge === 'left' \? 0 : window\.innerWidth - DOCK_WIDTH\)/);
assert.match(assistant, /dockLabel = english\(\) \? 'KRISTEN' : '克里斯滕'/);
assert.match(assistant, /function undockCharacter[\s\S]*window\.innerWidth - rect\.width - 8[\s\S]*window\.innerHeight - rect\.height - 8/);
assert.match(css, /\.assistant-character-trigger\.is-edge-docked[\s\S]*width:\s*44px[\s\S]*height:\s*132px/);
assert.match(css, /content:\s*attr\(data-dock-label\)[\s\S]*writing-mode:\s*vertical-rl/);
...
assert.match(mobile, /native-app \.utility-nav \.app-download-menu \{ display: none !important; \}/);
...
assert.match(html, /rhine-lab-assistant\.css\?v=0\.3\.3-ai5/);
assert.match(html, /rhine-lab-assistant\.js\?v=0\.3\.3-ai5/);
...
assert.match(worker, /rhine-lab-assistant\.css\?v=0\.3\.3-ai5/);
assert.ok(worker.includes(html.match(/css\/rhine-lab-mobile\.css\?v=[^"]+/)[0]), 'Service worker must cache the current mobile stylesheet');
assert.match(worker, /rhine-lab-assistant\.js\?v=0\.3\.3-ai5/);
