...
assert.match(assistant, /document\.addEventListener\('pointermove', moveCharacter, \{ passive: false \}\)/);
...
assert.match(html, /rhine-lab-assistant\.css\?v=0\.2\.5/);
assert.match(html, /rhine-lab-assistant\.js\?v=0\.2\.5/);
...
assert.match(worker, /rhine-lab-assistant\.css\?v=0\.2\.5/);
assert.match(worker, /rhine-lab-mobile\.css\?v=0\.2\.5d/);
assert.match(worker, /rhine-lab-assistant\.js\?v=0\.2\.5d/);
...
assert.match(main, /<h3>实验结果<\/h3>[\s\S]*<span class="status-chip">已填写<\/span>[\s\S]*已录入结果[\s\S]*修改结果/);
...
assert.match(main, /coldStorageSlotsHtml\(activeUnit, shelf, level, true\)/);
...
assert.match(appCss, /\.cold-storage-shelf-tabs\.vertical-racks/);
