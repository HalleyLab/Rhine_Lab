...
assert.match(worker, /rhine-lab-mobile\.css\?v=0\.2\.6b/);
...
assert.match(main, /function parseColdStorageLevels/);
assert.match(main, /function bindColdStorageBoxDrag/);
assert.match(main, /function bindColdStorageRackDrag/);
assert.match(main, /rackCount[\s\S]*rackOrder/);
assert.match(main, /storageRack/);
assert.match(main, /sourceShelf[\s\S]*targetShelf[\s\S]*box\.shelf = targetShelf;[\s\S]*box\.storageRack = targetRack;/);
assert.match(main, /targetLevel\.rackOrder\.length >= 8/);
assert.match(main, /data-cold-storage-racks data-rack-mode=/);
assert.match(main, /function detachColdStorageRack[\s\S]*rackNumbers[\s\S]*box\.storageRack = rackNumbers\.get/);
assert.match(main, /!remaining\.length[\s\S]*level\.mode = 'direct';[\s\S]*level\.rackOrder = \[1\]/);
assert.match(main, /data-cold-storage-rack-destination[\s\S]*targetUnitId[\s\S]*box\.storageUnitId = targetUnitId/);
assert.match(main, /drag\.tray\?\.remove\(\)/);
assert.match(main, /货架 2架 2x4/);
assert.match(main, /data-storage-unit/);
assert.match(main, /targetUnitId[\s\S]*targetShelf[\s\S]*other\.storageUnitId = sourceUnitId/);
assert.match(main, /coldStorageRacksHtml\(activeUnit, shelf, level, true\)/);
assert.match(main, /field\('orientation', '货架排列'/);
