...
assert.ok(topbar.indexOf('id="publicDemoBanner"') > topbar.indexOf('id="globalSearch"'));
assert.doesNotMatch(topbar, /topbar-context/);
assert.doesNotMatch(topbar.match(/<div class="public-demo-banner"[\s\S]*?<\/div>/)[0], /<a/);
const downloadMenuHtml = topbar.match(/<details class="app-download-menu"[\s\S]*?<\/details>/)[0];
assert.match(downloadMenuHtml, /<summary[^>]*>[\s\S]*下载应用/);
assert.match(downloadMenuHtml, /id="desktopDownloadButton"/);
assert.match(downloadMenuHtml, /href="\?app=1#dashboard"/);
...
assert.match(mobileCss, /\.topbar \.public-demo-banner \{[^}]*justify-self: end/);
assert.match(mobileCss, /html\.native-app \.utility-nav \.app-download-menu \{ display: none !important; \}/);
const downloadMenu = { open: true };
const toolbar = { utilityNav: {}, utilityNavToggle: { setAttribute() {} } };
const setUtilityNav = new Function('els', 'document', 'interfaceText', app.match(/function setUtilityNav\(open\) \{[\s\S]*?\n    \}/)[0] + '; return setUtilityNav;')(toolbar, { getElementById: () => downloadMenu }, String);
setUtilityNav(true);
assert.equal(downloadMenu.open, true);
setUtilityNav(false);
assert.equal(downloadMenu.open, false);
assert.equal(toolbar.utilityNav.hidden, true);
